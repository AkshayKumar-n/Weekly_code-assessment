...
def login_check(request):
    ...
    try:
        username = request.POST.get("fcode")
        password = request.POST.get("password")

        getUser = Faculty.objects.filter(fcode = username, password=password)
        fac_serializer = FacultySerializers(getUser, many=True)
        if(fac_serializer.data):
            for i in fac_serializer.data:
                x = i["name"]
                print(x)
            request.session['fname'] = x
